...
export const fetchRewardsDistributionConfig = async (api: ApiPromise): Promise<DistributionConfig> => {
  const result = await api.query.blockReward.rewardDistributionConfigStorage<RewardDistributionConfig>();
...
const estimateEraTokenIssuances = async ({ blockHeight,
  ...
  const formattedSevenEraAgoIssuance = Number(ethers.utils.formatEther(sevenEraAgoIssuance.toString()));
...
const formatStakerPendingRewards = ({ stakerInfo,
  ...
  return stakerInfo.map((it) => {
    ...
    const totalIssuance = eraTokenIssuances.find((that) => it.era === that.era)?.eraTokenIssuance || 0;
    const tvlPercentage = totalStaked / totalIssuance;
    const adjustableStakerPercentage = Math.min(1, tvlPercentage / idealDappsStakingTvl) * adjustablePercent;
...
export const estimatePendingRewards = async ({ api,
  ...
  try {
    ...
    const amtStakerPendingRewards = stakerPendingRewards.reduce((acc, it) => acc + it.pendingRewards, 0);
    const estAmtStaker = truncate(amtStakerPendingRewards, amtStakerPendingRewards > 1 ? 0 : 1);
    return { stakerPendingRewards: estAmtStaker };
  } catch (error) {
    console.error(error);
